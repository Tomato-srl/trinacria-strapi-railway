...
async function deleteOperatorFromOpenAI(operator) {
    try {
        ...
        const config = (0, openai_1.default)({ env: (key) => process.env[key] });
...
async function syncOperatorWithOpenAI(operator) {
    try {
        ...
        const fileResponse = await openaiService.uploadContentAsFile(config.assistantId || '', operatorData, filename);
